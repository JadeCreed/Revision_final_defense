...
// Farmer self-registration form connected to Django /register/farmer/ endpoint
// Fields: first_name, last_name, barangay, contact_number, rsbsa_number, email, password, confirm_password

import { useState } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { registerFarmer } from '../api/axios';

// Barangay options from your Django choices.py
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data && typeof data === 'object' && !data.error) {
        // Django DRF returns field errors as { field: ["error msg"] }
        // Flatten arrays to strings so they display correctly
        const flatErrors = {};
        ...
        // General error like { error: "..." }
        setApiError(data?.error || data?.non_field_errors?.[0] || 'Registration failed. Please try again.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="auth-card" style={{ maxWidth: '420px' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <select className={`form-input ${errors.barangay ? 'error' : ''}`}
              name="barangay" value={form.barangay} onChange={handleChange} required>
          ...
          <div className="form-group">
            ...
            <input className={`form-input ${errors.contact_number ? 'error' : ''}`}
              type="text" name="contact_number" placeholder="09123456789"
              maxLength={11} value={form.contact_number} onChange={handleChange} required />
          ...
          <div className="form-group">
            ...
            <input className={`form-input ${errors.rsbsa_number ? 'error' : ''}`}
              type="text" name="rsbsa_number" placeholder="e.g. 04-0432-000-0010"
              value={form.rsbsa_number} onChange={handleChange} required />
          ...
          <div className="form-group">
            ...
            <input className={`form-input ${errors.confirm_password ? 'error' : ''}`}
              type="password" name="confirm_password" placeholder="Confirm your password"

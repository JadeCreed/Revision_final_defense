...
const BrgyBeneficiaries = () => {
  ...
  const handleCreateEvent = async () => {
    ...
    const selectedSeedTypeName = selectedFS?.seed_type?.name?.toUpperCase() || '';

    const duplicate = events.find(ev => {
      const sameYear   = String(ev.year) === String(year);
      const sameSeason = (ev.season || '').toUpperCase() === season.toUpperCase();
      const sameType   = (ev.seed_type_name || '').toUpperCase() === selectedSeedTypeName;
      return sameYear && sameSeason && sameType;
    });

    if (duplicate) {
      setEventFormErrors({
        general: `A ${selectedFS?.seed_type?.name} program already exists for ${finalSeeds[0]?.season_display} ${year}. Select a different seed type, or wait for the admin to finalize seeds for a new season or year.`,
      });
      return;
    }
        ...
        <div style={{ padding: '1.25rem', animation: 'fadeIn 0.25s ease' }}>
          ...
          <button onClick={handleSaveEntry} disabled={saving}
            ...
            {saving ? 'Saving...'
              ...
              : <><CheckCircle size={18} /> Confirm Beneficiaries</>
            }
          </button>
        </div>
      )}

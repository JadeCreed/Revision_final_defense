...
const StatusBadge = ({ status }) => {
  const cfg = {
    ...
    APPROVED:  { bg: '#dcfce7', color: '#166634', label: 'Approved' },
...
// ── Inbred Panel ──
const InbredPanel = ({ entry, form, setForm, errors, setErrors, saving, onSave, inp }) => {
  const isEncoded  = entry?.is_distribution_encoded;
  const isLocked   = !entry;

  // Formula: 1ha = 2 bags (20kg each)
  ...
  if (isLocked) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '220px', gap: '0.875rem', padding: '2rem' }}>
        <div style={{ width: 56, height: 56, borderRadius: '50%', backgroundColor: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Lock size={24} color="#d1d5db" />
        </div>
        <p style={{ fontWeight: 700, color: '#9ca3af', margin: 0, textAlign: 'center', fontSize: '0.875rem' }}>Farmer not in this batch</p>
        <p style={{ color: '#d1d5db', fontSize: '0.75rem', margin: 0, textAlign: 'center' }}>This farmer has no approved Inbred batch entry.</p>
      </div>
    );
  }
  ...
  if (isEncoded) {
    return (
      <div style={{ padding: '0.25rem 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', padding: '0.75rem 1rem', backgroundColor: GREEN.light, borderRadius: '0.75rem', border: `1px solid ${GREEN.border}` }}>
          <CheckCircle size={18} color={GREEN.accent} />
          <span style={{ fontWeight: 700, color: GREEN.accent, fontSize: '0.875rem' }}>Distribution Complete</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
          {[
            ...
            <div key={label} style={{ backgroundColor: '#f9fafb', borderRadius: '0.5rem', padding: '0.5rem 0.75rem', border: '1px solid #f3f4f6' }}>
              <p style={{ fontSize: '0.6rem', color: '#9ca3af', fontWeight: 700, textTransform: 'uppercase', margin: '0 0 0.125rem', letterSpacing: '0.04em' }}>{label}</p>
              <p style={{ fontSize: '0.825rem', fontWeight: 700, color: '#1a1a1a', margin: 0 }}>{value}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      ...
      {/* From Beneficiaries */}
      <div style={{ backgroundColor: GREEN.light, border: `1px solid ${GREEN.border}`, borderRadius: '0.75rem', padding: '0.875rem' }}>
        <p style={{ fontWeight: 700, fontSize: '0.72rem', color: GREEN.accent, margin: '0 0 0.5rem', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
          <CheckCircle size={12} /> From Beneficiaries
        </p>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.3rem' }}>
          ...
          {suggestedBags !== null && (
            <span style={{ fontSize: '0.63rem', color: '#9ca3af', fontStyle: 'italic' }}>
        ...
        <div style={{ position: 'relative' }}>
          <input
            ...
            onChange={e => { setForm(p => ({ ...p, number_of_bags: e.target.value })); setErrors(p => ({ ...p, number_of_bags: '' })); }}
            placeholder={suggestedBags !== null ? `e.g. ${suggestedBags}` : 'e.g. 1'}
            style={{ ...inp(!!errors.number_of_bags || exceedsMax), paddingRight: liveKg !== null ? '5.5rem' : '0.875rem' }}
          />
          {liveKg !== null && (
            <span style={{
              position: 'absolute', right: '0.625rem', top: '50%', transform: 'translateY(-50%)',
              fontSize: '0.72rem', fontWeight: 700,
              color: exceedsMax ? '#dc2626' : GREEN.accent,
              backgroundColor: exceedsMax ? '#fee2e2' : GREEN.light,
        ...
        {exceedsMax && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', marginTop: '0.375rem', padding: '0.5rem 0.75rem', backgroundColor: '#fee2e2', borderRadius: '0.5rem', border: '1px solid #fca5a5' }}>
            <AlertCircle size={13} color="#dc2626" />
            <p style={{ fontSize: '0.72rem', color: '#dc2626', margin: 0, fontWeight: 600 }}>
              Exceeds limit. Max {maxBags} bag{maxBags !== 1 ? 's' : ''} ({maxBags * kgPerBag}kg) for {areaPlanted}ha.
            </p>
          </div>
        )}
      ...
      <button onClick={onSave} disabled={saving || exceedsMax} style={{ width: '100%', padding: '0.875rem', backgroundColor: saving || exceedsMax ? '#d1d5db' : GREEN.primary, color: 'white', border: 'none', borderRadius: '0.75rem', fontWeight: 700, fontSize: '0.875rem', cursor: saving || exceedsMax ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
...
const HybridPanel = ({ entry, form, setForm, errors, setErrors, saving, onSave, inp }) => {
  ...
  if (isLocked) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '220px', gap: '0.875rem', padding: '2rem' }}>
        ...
        <p style={{ fontWeight: 700, color: '#9ca3af', margin: 0, textAlign: 'center', fontSize: '0.875rem' }}>Farmer not in this batch</p>
        <p style={{ color: '#d1d5db', fontSize: '0.75rem', margin: 0, textAlign: 'center' }}>This farmer has no approved Hybrid batch entry.</p>
      </div>
    );
  }
  ...
  if (isEncoded) {
    return (
      <div style={{ padding: '0.25rem 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', padding: '0.75rem 1rem', backgroundColor: '#eff6ff', borderRadius: '0.75rem', border: '1px solid #bfdbfe' }}>
          <CheckCircle size={18} color="#1e40af" />
          <span style={{ fontWeight: 700, color: '#1e40af', fontSize: '0.875rem' }}>Distribution Complete</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
          {[
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      ...
      {/* From Beneficiaries */}
      <div style={{ backgroundColor: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '0.75rem', padding: '0.875rem' }}>
        <p style={{ fontWeight: 700, fontSize: '0.72rem', color: '#1e40af', margin: '0 0 0.5rem', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
          <CheckCircle size={12} /> From Beneficiaries
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.375rem' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '0.375rem', padding: '0.375rem 0.625rem' }}>
      ...
      <div>
        ...
        <div style={{ position: 'relative' }}>
          <input
            ...
            min="0"
            value={form.qty_bags ?? ''}
            onChange={e => { setForm(p => ({ ...p, qty_bags: e.target.value })); setErrors(p => ({ ...p, qty_bags: '' })); }}
            placeholder={suggestedBags !== null ? `e.g. ${suggestedBags}` : 'Number of seed bags received'}
            style={{ ...inp(!!errors.qty_bags || exceedsMax), paddingRight: liveKg !== null ? '5.5rem' : '0.875rem' }}
          />
          {liveKg !== null && (
            <span style={{
              position: 'absolute', right: '0.625rem', top: '50%', transform: 'translateY(-50%)',
              fontSize: '0.72rem', fontWeight: 700,
              color: exceedsMax ? '#dc2626' : '#1e40af',
              backgroundColor: exceedsMax ? '#fee2e2' : '#eff6ff',
              padding: '0.15rem 0.5rem', borderRadius: '999px',
              pointerEvents: 'none', whiteSpace: 'nowrap',
            }}>
              {liveKg}kg
        ...
        {exceedsMax && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', marginTop: '0.375rem', padding: '0.5rem 0.75rem', backgroundColor: '#fee2e2', borderRadius: '0.5rem', border: '1px solid #fca5a5' }}>
            <AlertCircle size={13} color="#dc2626" />
            <p style={{ fontSize: '0.72rem', color: '#dc2626', margin: 0, fontWeight: 600 }}>
              Exceeds limit. Max {maxBags} bag{maxBags !== 1 ? 's' : ''} ({maxBags * kgPerBag}kg) for {farmArea}ha.
            </p>
          </div>
        )}
      ...
      <button onClick={onSave} disabled={saving || exceedsMax} style={{ width: '100%', padding: '0.875rem', backgroundColor: saving || exceedsMax ? '#d1d5db' : '#1e40af', color: 'white', border: 'none', borderRadius: '0.75rem', fontWeight: 700, fontSize: '0.875rem', cursor: saving || exceedsMax ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
        <CheckCircle size={16} /> {saving ? 'Saving...' : 'Confirm Seed Distributed'}
      </button>
    </div>
  );
  ...
  return (
    <div style={{ backgroundColor: 'white', borderRadius: '1rem', border: '1px solid #e5e7eb', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
        ...
        {tabs.map(tab => {
          ...
          return (
            <button key={tab.id} type="button" onClick={() => onTabChange(tab.id)}
              style={{ flex: '1 1 160px', minHeight: '52px', borderRadius: '0.75rem', border: `1.5px solid ${isActive ? tab.border : '#e5e7eb'}`, backgroundColor: isActive ? tab.soft : 'white', color: isActive ? tab.accent : '#374151', padding: '0.625rem 0.75rem', textAlign: 'left', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '0.125rem', fontWeight: 700, boxShadow: isActive ? '0 2px 8px rgba(0,0,0,0.06)' : 'none' }}>
      ...
      <div style={{ padding: '1rem' }}>
        {activeTab === 'inbred' ? (
          <InbredPanel entry={inbredEntry} form={inbredForm} setForm={setInbredForm} errors={inbredErrors} setErrors={setInbredErrors} saving={inbredSaving} onSave={handleSaveInbred} inp={inp} />
        ) : (
          <HybridPanel entry={hybridEntry} form={hybridForm} setForm={setHybridForm} errors={hybridErrors} setErrors={setHybridErrors} saving={hybridSaving} onSave={handleSaveHybrid} inp={inp} />
        )}
      </div>
    </div>
  );
...
const BrgyDistribution = () => {
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      setEvents(evData);
      let totalApproved = 0, totalEncoded = 0;
  ...
  const handleSaveInbred = () => {
    ...
    setConfirmSnack({ title: `Confirm Inbred distribution for ${farmerDetail?.farmer?.last_name}, ${farmerDetail?.farmer?.first_name}?`, message: 'This will save the seed distribution record.', confirmLabel: 'Confirm' });
  ...
  const handleSaveHybrid = () => {
    ...
    setConfirmSnack({ title: `Confirm Hybrid distribution for ${farmerDetail?.farmer?.last_name}, ${farmerDetail?.farmer?.first_name}?`, message: 'This will save the seed distribution record.', confirmLabel: 'Confirm' });
  ...
  const doSave = async () => {
    ...
    if (type === 'inbred') {
      ...
      try {
        await updateEntry(entry.entry_id, {
          qty_bags:                  Number(inbredForm.number_of_bags),
    ...
    if (type === 'hybrid') {
      ...
      try {
        await updateEntry(entry.entry_id, { qty_bags: Number(hybridForm.qty_bags) });
  ...
  const openReport = async (event) => {
    ...
    try {
      ...
      if (approved.length > 0) { setReportBatchId(approved[0].id); await loadReportBatch(approved[0].id); }
      ...
      {view === 'landing' && (
        <div style={{ padding: '1.25rem', animation: 'fadeIn 0.25s ease' }}>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '1rem', padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', marginBottom: '1.25rem', border: '1px solid #f3f4f6' }}>
            ...
            <p style={{ fontSize: '0.75rem', color: '#9ca3af', margin: '0 0 0.75rem' }}>Search farmers who have been approved in Beneficiaries batches.</p>
            <div style={{ position: 'relative' }}>
            ...
            {!landingSearching && landingSearch.length >= 2 && landingResults.length === 0 && (
              <p style={{ fontSize: '0.78rem', color: '#9ca3af', margin: '0.5rem 0 0', textAlign: 'center', padding: '0.75rem', backgroundColor: '#f9fafb', borderRadius: '0.5rem' }}>No farmers found in approved batches.</p>
            )}
          ...
          {filteredEvents.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
              {filteredEvents.map((event, idx) => {
                ...
                return (
                  <div key={event.id} className="prog-card" onClick={() => openReport(event)}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.875rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                      <div>
                        <div style={{ display: 'flex', gap: '0.375rem', marginBottom: '0.375rem', flexWrap: 'wrap' }}>
                          ...
                          {event.variety_name && <span style={{ fontSize: '0.7rem', color: '#6b7280', backgroundColor: '#f9fafb', padding: '0.15rem 0.5rem', borderRadius: '999px', border: '1px solid #e5e7eb' }}>{event.variety_name}</span>}
                        </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: '#9ca3af', alignItems: 'center' }}>
                      ...
                      <span style={{ color: tagColor, fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.25rem' }}>View Masterlist <ChevronRight size={12} /></span>
                    </div>
                  </div>
                );
              })}
          ...
          {farmerDetailLoading ? (
            ...
            <>
              ...
              <TabbedSeedPanel
                ...
                inbredForm={inbredForm}       setInbredForm={setInbredForm}
                inbredErrors={inbredErrors}   setInbredErrors={setInbredErrors}
                inbredSaving={inbredSaving}   handleSaveInbred={handleSaveInbred}
                hybridForm={hybridForm}       setHybridForm={setHybridForm}
                hybridErrors={hybridErrors}   setHybridErrors={setHybridErrors}
                hybridSaving={hybridSaving}   handleSaveHybrid={handleSaveHybrid}
                inp={inp}
              />
            ...
            <div style={{ backgroundColor: 'white', borderRadius: '1rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', overflow: 'hidden', border: '1px solid #f3f4f6' }}>
              <div style={{ padding: '0.875rem 1.25rem', borderBottom: '2px solid #e5e7eb', backgroundColor: GREEN.light, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
                <span style={{ fontSize: '0.8rem', fontWeight: 700, color: GREEN.accent }}>
                  Batch {reportBatchData?.batch_number} <StatusBadge status={reportBatchData?.status} />
                </span>
                      ...
                      {reportBatchData.entries.map((entry, idx) => {
                        ...
                        return (
                          <tr key={entry.id} style={{ backgroundColor: !entry.qty_bags ? '#fffbeb' : idx % 2 === 0 ? 'white' : '#fafafa' }}>
                            ...
                            <td style={{ ...td, textAlign: 'center' }}>{entry.qty_bags ? entry.qty_bags : <span style={{ backgroundColor: '#fef9c3', color: '#854d0e', padding: '0.15rem 0.5rem', borderRadius: '999px', fontSize: '0.6rem', fontWeight: 700 }}>Pending</span>}</td>

...
const normalizePhase = (value) => {
  ...
  const matched = Object.keys(PHASE_MAP).find(key => key.toLowerCase() === phase.toLowerCase());
...
const normalizeBrgy = (value) =>
  value?.toString().trim().replace(/[-_]+/g, ' ').replace(/\s+/g, ' ').toLowerCase();

const BRGY_FEATURES = LucbanGIS.features.filter(
  f => ALLOWED_BRGYS.some(b => normalizeBrgy(f.properties?.ADM4_EN) === normalizeBrgy(b))
);
...
const PhaseIcon = ({ phaseKey, size = 18 }) => {
  const icons = {
    'Seed Distribution':  MapPin,
    'Crop Establishment': Activity,
    'Tillering':          TrendingUp,
    'Flowering':          Layers,
    'Ripening':           CheckCircle,
    'Harvesting':         RefreshCw,
  };
  const Icon = icons[phaseKey] || MapPin;
  return <Icon size={size} color={phaseColor(phaseKey)} />;
};
...
const OverviewPanel = ({ plots, summary, animate }) => {
  const currentFarmers = summary?.current_farmers ?? summary?.total_farmers ?? new Set(plots.map(p => p.farmer)).size;
  const totalApprovedFarmers = summary?.total_approved_farmers ?? summary?.total_farmers ?? currentFarmers;
  const currentBarangays = summary?.current_active_barangays ?? summary?.barangays?.length ?? new Set(plots.map(p => p.barangay).filter(Boolean)).size;
  const totalApprovedBarangays = summary?.total_active_barangays ?? summary?.barangays?.length ?? currentBarangays;
  ...
  const activeDistribution = distribution.filter(ph => ph.count > 0);
  const dominant = activeDistribution.length > 0 ? [...activeDistribution].sort((a, b) => b.count - a.count)[0] : null;
    ...
    <div style={{ padding: '0 1.25rem 1.5rem', animation: animate ? 'gis-fadeSlide 0.35s ease' : 'none' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
          <div>
            <p style={{ margin: 0, fontSize: '1.8rem', fontWeight: 800, color: 'white', lineHeight: 1 }}>
              {`${currentFarmers.toLocaleString()}/${totalApprovedFarmers.toLocaleString()}`}
            </p>
            <p style={{ margin: '0.25rem 0 0', fontSize: '0.72rem', color: 'rgba(255,255,255,0.65)', fontWeight: 600 }}>Total Farmers</p>
          </div>
          <div>
            <p style={{ margin: 0, fontSize: '1.8rem', fontWeight: 800, color: 'white', lineHeight: 1 }}>
              {`${currentBarangays.toLocaleString()}/${totalApprovedBarangays.toLocaleString()}`}
            </p>
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '1.25rem', padding: '1.1rem 1.15rem', border: '1px solid #e2e8f0', marginBottom: '1rem' }}>
        ...
        {activeDistribution.length === 0 ? (
          ...
          activeDistribution.sort((a, b) => b.count - a.count).map(ph => (
      ...
      {summary?.seed_breakdown && Object.keys(summary.seed_breakdown).length > 0 && (
        <div style={{ backgroundColor: 'white', borderRadius: '1.25rem', padding: '1.1rem 1.15rem', border: '1px solid #e2e8f0', marginBottom: '1rem' }}>
          <p style={{ margin: '0 0 0.875rem', fontSize: '0.78rem', fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Seed Source Summary
          </p>
            ...
            {Object.entries(summary.seed_breakdown).map(([src, data]) => {
              ...
              return (
                <div key={src} style={{ backgroundColor: `${color}0d`, borderRadius: '0.875rem', padding: '0.75rem', border: `1px solid ${color}22`, textAlign: 'center' }}>
                  <p style={{ margin: 0, fontSize: '1.2rem', fontWeight: 800, color }}>{data.total_farmers}</p>
                  <p style={{ margin: '0.2rem 0 0', fontSize: '0.62rem', color: '#64748b', fontWeight: 600, lineHeight: 1.3 }}>{data.label}</p>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div style={{ padding: '1rem 1.25rem', backgroundColor: '#f8fafc', borderTop: '1px solid #f1f5f9', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
      ...
      <div style={{ backgroundColor: '#f8fafc', borderRadius: '1rem', padding: '0.875rem', border: '1px solid #e2e8f0' }}>
        ...
        <p style={{ margin: 0, fontSize: '0.78rem', fontWeight: 700, color: '#16a34a', lineHeight: 1.4 }}>AT Monitoring<br /><span style={{ color: '#64748b', fontSize: '0.68rem', fontWeight: 400 }}>(Latest visit status)</span></p>
      </div>
    </div>
  );
};
...
const UtilizationOverviewPanel = () => (
  <div style={{ padding: '0 1.25rem 1.5rem' }}>
    <div style={{ backgroundColor: '#f8fafc', borderRadius: '1.25rem', padding: '2rem', textAlign: 'center', border: '1px solid #e2e8f0' }}>
      <TrendingUp size={28} color='#1a4d1a' style={{ marginBottom: '0.5rem' }} />
...
// ─── BARANGAY PANEL ───────────────────────────────────────────
const BarangayPanel = ({ barangayName, plots, approvedCounts, onBack, animate }) => {
  const brgyPlots = useMemo(() => plots.filter(p => p.barangay === barangayName), [plots, barangayName]);

  const currentFarmers = useMemo(() => new Set(brgyPlots.map(p => p.farmer)).size, [brgyPlots]);
  const phaseCounts = useMemo(() => {
    const counts = {};
    brgyPlots.forEach(p => {
      const k = normalizePhase(p.land_type);
  ...
  const totalApproved = approvedCounts?.[barangayName] ?? brgyPlots[0]?.total_approved_in_brgy ?? currentFarmers;
  ...
  return (
    <div style={{ animation: animate ? 'gis-fadeSlide 0.35s ease' : 'none' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.875rem' }}>
          ...
          {dominantCfg ? (
            ...
            <span style={{ marginLeft: 'auto', backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: '999px', padding: '0.2rem 0.75rem', fontSize: '0.7rem', fontWeight: 700, color: 'rgba(255,255,255,0.9)', whiteSpace: 'nowrap' }}>
              No phase data yet
            </span>
          )}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.6rem', marginTop: '1rem' }}>
          {[
            { label: 'Farmers', value: totalApproved ? `${currentFarmers}/${totalApproved}` : `${currentFarmers}` },
            { label: 'Area', value: (totalHa > 0 || totalAreaInBrgy > 0) ? `${totalHa.toFixed(1)}/${totalAreaInBrgy.toFixed(1)} ha` : '—' },
      ...
      {/* Seed source grouped phase breakdown */}
      <div style={{ padding: '1.15rem 1.25rem', borderBottom: '1px solid #f1f5f9' }}>
        ...
        {(() => {
          ...
          const totalFarmers = brgyPlots.length;
            ...
            return (
              <div style={{ textAlign: 'center', padding: '1.5rem 0', color: '#94a3b8' }}>
                ...
                <p style={{ margin: '0.25rem 0 0', fontSize: '0.75rem', color: '#cbd5e1' }}>
                  Data appears after AT monitoring visits are recorded.
                </p>
              </div>
            );
          }
          ...
          return Object.entries(groups).map(([srcKey, grp]) => (
            <div key={srcKey} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                ...
                <span style={{ marginLeft: 'auto', backgroundColor: `${grp.color}18`, color: grp.color, borderRadius: '999px', padding: '0.1rem 0.5rem', fontSize: '0.65rem', fontWeight: 700 }}>
              ...
              {Object.entries(grp.phases).map(([phase, count]) => {
                const cfg = PHASE_MAP[phase] || { color: '#9ca3af', label: phase };
                const pct = Math.round((count / Math.max(1, totalFarmers)) * 100);
                return (
...
const GisMap = () => {
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    try {
      const [plotsRes, sumRes, brgyRes] = await Promise.allSettled([
        getGisPlots(),
        getMapSummary(),
        getGisBarangays(),
      ]);
      if (plotsRes.status === 'fulfilled') {
        ...
        if (Array.isArray(data)) {
          ...
          setPlots(data.plots || []);
          setApprovedCounts(data.approved_counts || {});
        }
      }
      if (sumRes.status === 'fulfilled') setSummary(sumRes.value.data);
      if (brgyRes.status === 'fulfilled') setBarangays(brgyRes.value.data || []);
      const allFailed = [plotsRes, sumRes, brgyRes].every(item => item.status === 'rejected');
      if (allFailed) {
        showToast('Failed to load GIS data.', 'error');
      }
    } catch {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      Promise.allSettled([
        ...
        const [plotsRes, sumRes, brgyRes] = results;
        if (plotsRes.status === 'fulfilled') {
          const data = plotsRes.value.data;
          if (Array.isArray(data)) {
            setPlots(data);
  ...
  const filteredPlots = useMemo(() => plots.filter(p => {
    ...
    const normalizedBrgy = normalizeBrgy(p.barangay);
    if (q && !(p.farmer_name?.toLowerCase().includes(q) || normalizedBrgy?.includes(q))) return false;
    if (filterBrgy && normalizedBrgy !== normalizeBrgy(filterBrgy)) return false;
    return true;
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden', padding: '1rem', gap: '1rem' }}>
          ...
          <div style={{
            ...
            <div style={{ padding: '0.6rem 1.1rem', backgroundColor: '#f8fafc', borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <span style={{ fontSize: '0.7rem', color: '#94a3b8' }}>Data is updated in real-time based on AT monitoring visits.</span>
            </div>
          </div>
...
//   const totalAreaInBrgy = brgyPlots[0]?.total_approved_area_ha_in_brgy || 0;
...
//         <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.6rem', marginTop: '1rem' }}>
//           {[
//             { label: 'Farmers', value: `${brgyPlots.length}/${totalApproved}` },
//             { label: 'Area', value: (totalHa > 0 || totalAreaInBrgy > 0) ? `${totalHa.toFixed(1)}/${totalAreaInBrgy.toFixed(1)} ha` : '—' },
//             { label: 'Plots', value: brgyPlots.length },
//           ].map(item => (
//             <div key={item.label} style={{ backgroundColor: 'rgba(255,255,255,0.12)', borderRadius: '0.75rem', padding: '0.6rem 0.75rem', textAlign: 'center' }}>

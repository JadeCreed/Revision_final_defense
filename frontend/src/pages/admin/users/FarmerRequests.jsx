...
const FarmerRequests = () => {
  ...
  const fetchFarmers = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      if (isInitial) {
        // ── INITIAL LOAD ──
        // Compare current items against localStorage seen IDs.
        // Items NOT in seen IDs → show NEW badge.
        // This makes badges survive navigation and page refresh.
        ...
        // ── POLL UPDATE ──
        // New = in current fetch but NOT in previous fetch.
  ...
  const handleAction = async (userId, action) => {
    ...
    try {
      ...
      await fetchFarmers(false); // refresh table
            ...
            <thead>
              <tr style={{ backgroundColor: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                {[
                  ...
                  <th
                    key={col}
                    style={{ padding: '0.875rem 1rem', textAlign: 'left', fontWeight: '600', color: '#374151', minWidth: w, whiteSpace: 'nowrap' }}
                  >
                    {col}
                  </th>
                ))}
              ...
              {loading ? (
                ...
                farmers.map((farmer, idx) => {
                  ...
                  return (
                    ...
                    >
                      {/* RSBSA number */}
                      <td style={{ padding: '0.875rem 1rem', color: '#6b7280', fontFamily: 'monospace', minWidth: COL_WIDTHS.rsbsa }}>
                        {farmer.rsbsa_number || '—'}
                      </td>
                      ...
                      {/* Barangay */}
                      <td style={{ padding: '0.875rem 1rem', color: '#6b7280', minWidth: COL_WIDTHS.barangay }}>
                        {farmer.barangay || '—'}
                      </td>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '1rem', padding: '2rem', maxWidth: '400px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <p style={{ color: '#6b7280', marginBottom: '1.5rem', fontSize: '0.9rem' }}>
              ...
              <strong>{confirmModal.action === 'APPROVED' ? 'approve' : 'reject'}</strong>{' '}
            ...
            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  ...
                  backgroundColor: confirmModal.action === 'APPROVED' ? '#2d6a2d' : '#dc2626',
                  color:           'white',
                  border:          'none',
                  borderRadius:    '0.5rem',
                  cursor:          actionLoading[confirmModal.userId] ? 'not-allowed' : 'pointer',
                  fontWeight:      '600',
                  opacity:         actionLoading[confirmModal.userId] ? 0.7 : 1,
                }}
              >
